...
const ClientesModule = {
    ...
    // Configurar botones de clientes - Refactorizado con EventManager
    setupClientButtons: function() {
        FacturaExpress.EventManager.attachHandlers(this, [
            { selector: '.ver-perfil', handler: this.handleVerPerfil },
            { selector: '.nueva-venta', handler: this.handleNuevaVenta }
        ]);
    },

    handleVerPerfil: function(e) {
        const id = parseInt(e.target.closest('.ver-perfil')?.dataset.id);
        if (id) this.verPerfilCliente(id);
    },

    handleNuevaVenta: function(e) {
        const btn = e.target.closest('.nueva-venta');
