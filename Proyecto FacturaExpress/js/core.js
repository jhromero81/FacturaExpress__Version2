...
const FacturaExpress = {
    ...
    storage: {
        ...
        clearSession: () => {
            const prefs = FacturaExpress.storage.get('prefs');
            const config = FacturaExpress.storage.get('config');
            
            localStorage.clear();
            sessionStorage.clear();
            
            if (prefs) FacturaExpress.storage.set('prefs', prefs);
            if (config) FacturaExpress.storage.set('config', config);
        }
    },
    ...
    SampleData: {
        ...
        generarFacturasEjemplo: function(cantidad = 25) {
            ...
            for (let i = 1; i <= cantidad; i++) {
                ...
                const monto = Math.floor(Math.random() * 500000) + 50000;
