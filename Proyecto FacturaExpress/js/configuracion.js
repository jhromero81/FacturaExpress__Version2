...
const ConfiguracionModule = {
    ...
    renderConfiguracion: function() {
        ...
        if (resolucionSpan) {
            resolucionSpan.innerHTML = `<i class="material-icons tiny" style="font-size:14px;">receipt</i> ${this.state.configuracion.fiscal.resolucionDIAN}`;
        }
    ...
    cambiarContrasena: function() {
        const actual = prompt('Ingrese su contraseña actual:');
        if (!actual) return;
    ...
    verDispositivosConectados: function() {
        ...
        const mensaje = '📱 Dispositivos conectados:\n' + dispositivos.map((d, i) => `${i + 1}. ${d}`).join('\n');
        alert(mensaje);
    },
