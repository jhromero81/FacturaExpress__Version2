...
const DashboardModule = {
    ...
    // Inicializar módulo
    init: function() {
        console.log('Módulo del Dashboard inicializado');
        this.loadData();
    ...
    loadData: function() {
        // Cargar facturas para calcular estadísticas reales
    ...
    calcularVentasSemanales: function(facturas) {
        ...
        for (let i = 0; i < 7; i++) {
            ...
            const ventasDia = facturas.filter(f => {
                const fechaFactura = new Date(f.fecha);
                return fechaFactura >= inicioDia && fechaFactura < finDia;
            }).reduce((sum, f) => sum + f.total, 0);
            
            ventasPorDia[i] = ventasDia;
        }
        
        this.state.ventasSemanales = ventasPorDia;
    },
    ...
    actualizarEstadisticas: function() {
        ...
        if (variaciones.length >= 3) {
            // Variación ventas
            variaciones[0].innerHTML = `<i class="material-icons" style="font-size:14px;">${stats.variacionVentas >= 0 ? 'arrow_upward' : 'arrow_downward'}</i> ${Math.abs(stats.variacionVentas)}% vs ayer`;
            variaciones[0].className = `stat-change ${stats.variacionVentas >= 0 ? 'change-up' : 'change-down'}`;
            
            // Variación facturas
            variaciones[1].innerHTML = `<i class="material-icons" style="font-size:14px;">${stats.variacionFacturas >= 0 ? 'arrow_upward' : 'arrow_downward'}</i> ${Math.abs(stats.variacionFacturas)}% vs ayer`;
